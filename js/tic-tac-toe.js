...
function CellClicked(x, y) {
  if (!gameActie) {
    // Als het spel niet bezig is,r eturn
    return;
  }
  ...
  if (boardState[cellIndex] === "") {
    ...
    if (currentPlayer === startPlayer) {
      ...
      clickedCell.style.textShadow =
        "0 0 5px rgba(255, 0, 0, 0.7), 0 0 10px rgba(255, 0, 0, 0.5), 0 0 15px rgba(255, 0, 0, 0.3)";
    } else {
      clickedCell.style.color = "blue";
      clickedCell.style.textShadow =
        "0 0 5px rgba(0, 0, 255, 0.7), 0 0 10px rgba(0, 0, 255, 0.5), 0 0 15px rgba(0, 0, 255, 0.3)";
    }
...
function highlightWinningCells(cells) {
  cells.forEach((index) => {
    const cell = document.querySelector(
      `#cell-${Math.floor(index / 3)}-${index % 3}`
    );
